...
import 'dart:convert';
import '../base/error/error_model.dart';
...
class ResponseParser<T extends BaseModel> {
  ...
  R? fromList<R>({T? model}) {
    if (isList) {
      ...
      try {
        ...
        return ErrorModel(message: response) as R;
      } catch (e) {
        throw Exception(e);
      }
    }
  ...
  T? fromMap({T? model}) {
    if (isMap) {
      ...
      try {
        return ErrorModel(message: response) as T;
      } catch (e) {
        throw Exception(e);
      }
    }
  ...
  List<T>? fromStringList({T? model}) {
    if (isString) {
      ...
      try {
        return ErrorModel(message: response) as List<T>;
      } catch (e) {
        throw Exception(e);
      }
    }
